...
public class Desk
{
  ...
  public Desk(float x, float y,float w ,float h)
  {
  ...
  public void collisionDirection(Nolan nolan)
  {
    ...
    this.posBottom = (nolan.getY() + nolan.getHeight()/2 >= getY() + height);
    this.posRight = (nolan.getX() <= getX() + width);
    this.posLeft = (nolan.getX() + nolan.getWidth() >= getX());
    if (this.posTop) {
